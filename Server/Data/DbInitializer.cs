...
namespace hopkins.tech.Server.Data
{
    public class DbInitializer
    {
        ...
        public static void Initialize(HopkinsTechContext context, IConfiguration configuration)
        {
            ...
            string? username = configuration.GetSection("AdminUser:Username").Value;
            string? password = configuration.GetSection("AdminUser:Password").Value;

            if (username != null && password != null)
            {
                Tuple<byte[], byte[]> saltAndHash = CreatePasswordHash(password);

                context.Users.Add(new User { Id= new Guid(), Username = username, PasswordSalt = saltAndHash.Item1, PasswordHash = saltAndHash.Item2, Role = "Administrator" });
                context.SaveChanges();
            }
        }
        ...
        private static Tuple<byte[], byte[]> CreatePasswordHash(string password)
        {
            ...
            {
                return new Tuple<byte[], byte[]>(hmac.Key, hmac.ComputeHash(System.Text.Encoding.UTF8.GetBytes(password)));
            }
        }
    }
}